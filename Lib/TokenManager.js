...
let verifyToken = async function (token, flag, callback) {
	...
	try{
		if (token.type === UniversalFunctions.CONFIG.APP_CONSTANTS.DATABASE.USER_TYPE.ADMIN) {
			user = await getRequired(Modal.Admins, {_id: mongoose.Types.ObjectId(token._id),tokenTime:token.tokenTime}, {__v: 0, password: 0}, {lean: true});
		} else if (token.type === UniversalFunctions.CONFIG.APP_CONSTANTS.DATABASE.USER_TYPE.USER) {

			user = await getRequired(Modal.Users, {_id: mongoose.Types.ObjectId(token._id),tokenTime:token.tokenTime}, {__v: 0}, {lean: true});
		}
	}catch(err){
        console.log(err)
	}
