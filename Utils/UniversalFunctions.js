...
const fs = require('fs')
const baseUrl = "http://45.232.252.32:8000"
// const baseUrl = "http://192.168.100.129:8002"
...
//upload the image
const imageUploadLocal = function (data) {
    return new Promise((resolve, reject) => {
        if (data) {
            const name = new Date().getTime() + "" + data.image.hapi.filename;
            ...
            data.image.on('end', (err) => {
                const ret = {
                    ...
                    path: path,
                    urls: {
                        original: baseUrl+"/uploads/"+name,
                        thumbnail: baseUrl+"/uploads/"+name,
                        type: "image"
                    }
                }
                resolve(ret);
...
module.exports = {
    ...
    imageUploadLocal: imageUploadLocal
}